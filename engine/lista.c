...
void lista_set_index(lista_t *l, int index){
	/* Setea la posición en la lista. Si el index
	   pasado por parámetro es mayor a la cantidad
	   de elementos, queda seteado el último elemento */
	if(index > l->size - 1)
		l->actual = l->last;
	else {
		l->actual = l->first;
